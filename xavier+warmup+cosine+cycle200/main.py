...
parser.add_argument('--warmup', '-w', default=None, type=int,
                    help='warm up epoch')
...
# scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=200, T_mult=1, eta_min=0, last_epoch=-1, verbose=False)
T_max = 200	# 周期
lr_max = 0.1	# 最大值
lr_min = 0	# 最小值
# Warm up + Cosine Anneal
lambda0 = lambda cur_iter: cur_iter / args.warmup if  cur_iter < args.warmup else \
        (lr_min + 0.5*(lr_max-lr_min)*(1.0+math.cos( (cur_iter-args.warmup)/(T_max-args.warmup)*math.pi)))/0.1
